  ...
  Dir.chdir(ARGV[0]) do
    (Dir.glob "*.json").each do |file|
      ...
      csv.write(
	    ...
	    data["other"]["defense"].strip.gsub('""', '"').gsub(',', ';').delete("\n") + ', ' +
	    data["other"]["fuel"].strip.gsub('""', '"').gsub(',', ';').delete("\n") + ', ' +
	    data["other"]["strategy-notes"].strip.gsub('""', '"').gsub(',', ';').delete("\n") + ', ' +
	    data["other"]["notes"].strip.gsub('""', '"').gsub(',', ';').delete("\n") + "\n")
	end
    end
  end
end
